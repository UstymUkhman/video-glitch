...
export default class VideoGlitch {
  ...
  _setShaderMaterial() {
    this.shaderMaterial = new THREE.ShaderMaterial({
      ...
      uniforms: {
        direction: {type: 'v2', value: new THREE.Vector2(0.0, 0.0)},
        iChannel0: {type: 't', value: 0.0, texture: 0.0}
      }
    });
  }
  ...
  _render() {
    if (this.video.readyState === this.video.HAVE_ENOUGH_DATA) {
      ...
      this.imageData = context.getImageData(0, 0, this.WIDTH, this.HEIGHT).data;
      this.shaderMaterial.uniforms.direction.value = new THREE.Vector2(0.0, 0.0);
  ...
  _updateGeometry() {
    ...
    for (let i = 0; i < this.particles; i++) {
      ...
      if (this._getPixelSize(x, y)) {
        ...
        if (i % 2 === 0) {
          this.shaderMaterial.uniforms.direction.value = new THREE.Vector2(3.0, 0.0);
        } else {
          this.shaderMaterial.uniforms.direction.value = new THREE.Vector2(0.0, 3.0);
        }
      }
